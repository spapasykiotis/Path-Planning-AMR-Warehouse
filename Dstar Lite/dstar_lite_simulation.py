"""
D_star_Lite 2D multirobot simulation
@author: Spyros Papasykiotis
"""

import matplotlib.pyplot as plt
import dstar_lite as d_star_lite
import math
import random
import pandas as pd
        ...
        for j, (other_planner_data, _) in enumerate(all_robot_data):
            ...
            if is_nearby(robot1,robot2, radius=0.5):
                ...
                if nodes_remaining_i  <=0 and nodes_remaining_j <= 0:
                    ...
                    all_robot_data[closer_robot][0].Plot.plot_path(new_path[:], line='--', lb=f'Robot {closer_robot + 1} Updated Path')
...
def main():
    ...
    for i, robot_data in enumerate(robots):
        ...
        dstar_lite = d_star_lite.DStar(start, goal, "euclidean")
        dstar_lite.run()

        all_robot_data.append((dstar_lite,  robot_data))    # Add planner data to global robot variable    
        max_nodes = max(max_nodes, len(dstar_lite.path))  # Update num_iterations max_nodes_in_final_path
        
        if show_plot:  
            dstar_lite.Plot.plot_path(dstar_lite.path, lb=f'Robot {i + 1} Path')
        
        if show_stats:
            print(f'Robot {i + 1} Statistics:')
            d_star_lite.show_statistics(dstar_lite)
            print()       
            
    # Plotting grid once (every dstar instance has same env)
    dstar_lite.Plot.plot_grid_test("Dynamic A* Lite", plt.gca()) 
    ...
    if time_simulation:
        ...
        if show_stats:
            ...
            for i, (planner_data, robot_data) in enumerate(final_robot_data):
                ...
                print(f'  Path length: {planner_data.path_length}')
